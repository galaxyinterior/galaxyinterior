...
export function NotificationBell() {
  ...
  useEffect(() => {
    const q = query(
      ...
      where('userId', '==', 'ADMIN'), // Admin notifications go to 'ADMIN'
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="relative p-2 text-gray-500 hover:text-brand-navy hover:bg-gray-100 rounded-full transition-colors"
      >
        <Bell size={24} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 className="font-black text-brand-navy">Admin Notifications</h3>
            {unreadCount > 0 && (
              <button 
